...
const Register = () => {
  ...
  const { signup } = useAuth(); // <-- Destructured signup function
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // 3. Delegate to AuthContext (Handles API call and sets state automatically)
      await signup(dataToSend);

      // 4. Send directly to dashboard (Dashboard handles the "Start Onboarding" UI)
      navigate("/dashboard");
    } catch (err) {
      setError(
