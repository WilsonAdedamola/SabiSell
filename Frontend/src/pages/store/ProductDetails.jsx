...
import { 
  ...
  ShieldCheck, Truck, ChevronRight, Check
} from "lucide-react";
    ...
    const fetchProductAndStore = async () => {
      ...
      try {
        ...
        if (storeLink) {
          ...
          const foundProduct = storeData.products?.find(p => 
    ...
    <div className="min-h-screen bg-[#FDFDFB] font-sans pb-16">
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50 pt-4 pb-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
          <Link to={basePath || "/"} className="flex items-center gap-3">
            {store?.logoUrl ? (
              <img src={store.logoUrl} alt="Logo" className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover" />
            ) : null}
          ...
          <div className="flex items-center gap-4 sm:gap-6 text-gray-600">
            <Link to={`${basePath}/cart`} className="relative p-2 group flex items-center">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8">
        ...
        <nav className="flex items-center gap-2 text-xs font-bold text-gray-400 mb-6 sm:mb-8 uppercase tracking-wider overflow-x-auto whitespace-nowrap hide-scrollbar">
          <Link to={basePath || "/"} className="hover:text-gray-900 transition-colors">Home</Link>
          ...
          <span className="text-gray-900 truncate max-w-[200px] sm:max-w-sm">{product.name}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-7 flex flex-col gap-4">
            <div className="w-full aspect-[4/5] sm:aspect-[3/4] lg:aspect-square bg-[#F5F2ED] rounded-3xl overflow-hidden relative border border-gray-100">
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                {isOutOfStock && <span className="bg-red-500 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">Out of Stock</span>}
                {!isOutOfStock && product.stockQuantity < 5 && <span className="bg-orange-500 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">Low Stock</span>}
              </div>
            </div>
            ...
            {product.imageUrls?.length > 1 && (
              <div className="flex items-center gap-3 overflow-x-auto hide-scrollbar pb-2">
                {product.imageUrls.map((img, idx) => (
                  ...
                  >
                    <img src={img} alt={`Thumbnail ${idx+1}`} className="w-full h-full object-cover bg-[#F5F2ED] mix-blend-multiply" />
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="lg:col-span-5 flex flex-col lg:py-4">
            ...
            <div className="mb-6">
              <h1 className="text-2xl sm:text-4xl font-serif text-gray-900 leading-tight mb-4">{product.name}</h1>
            ...
            <div className="space-y-6 mb-8">
              <div>
                ...
                {quantity >= product.stockQuantity && !isOutOfStock && (
                  <p className="text-[10px] font-bold text-orange-500 mt-2">Maximum stock reached.</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-gray-100">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-gray-900">Nationwide Delivery</h4>
                  <p className="text-[10px] text-gray-500 mt-0.5">We ship to all states in Nigeria.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-500 mt-0.5">Powered safely by Paystack.</p>
                </div>
              </div>
            </div>
