...
import { 
  ...
  ChevronDown, Check, Loader2, AlertCircle, Image as ImageIcon
} from "lucide-react";
...
const AddEditProduct = () => {
  ...
  const [isFetching, setIsFetching] = useState(isEditMode); // Loading state for initial fetch
  ...
  const [existingImages, setExistingImages] = useState([]); // Tracks images already in the DB
  ...
  useEffect(() => {
    if (isEditMode) {
      const fetchProductData = async () => {
        try {
          // You will need a GET /api/products/:id route in your backend for this!
          ...
          if (product.imageUrls && product.imageUrls.length > 0) {
            ...
            setExistingImages(product.imageUrls); // Keep track of what was already there
          }
  ...
  const handleImageUpload = (e) => {
    ...
    if (imagePreviews.length + files.length > 5) {
      setError("You can only have up to 5 images per product.");
      return;
    }
  ...
  const removeImage = (indexToRemove) => {
    // If they remove an existing DB image, we track it by filtering our preview array
    setImagePreviews(prev => prev.filter((_, index) => index !== indexToRemove));
    
    // Also remove from the new files array if it was a newly added image
    // This math prevents removing the wrong file if mixed existing/new images are present
    ...
    if (isNewImage) {
      ...
      // It was an existing image that got deleted. Update our existing tracker.
  ...
  const handlePublish = async (forceDraft = false) => {
    ...
    try {
      ...
      // Send the URLs of the images the user decided to KEEP
      ...
      // Dynamic API Call: PUT if editing, POST if adding
      if (isEditMode) {
            ...
            <div className="animate-in slide-in-from-right-4 duration-300">
              ...
              <div className="mb-8">
                <div className="flex justify-between items-end mb-3">
                  ...
                  <span className={`text-xs font-bold px-2 py-1 rounded-md ${imagePreviews.length === 5 ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500'}`}>
                    {imagePreviews.length}/5
                  </span>
                </div>
                
                <div className="flex gap-3 overflow-x-auto hide-scrollbar pb-2">
                  {/* Image Previews (Shows both existing from DB and newly uploaded) */}
