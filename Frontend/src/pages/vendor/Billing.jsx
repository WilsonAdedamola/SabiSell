...
const Billing = () => {
  ...
  const handleUpgrade = async (newPlan) => {
    if (vendor?.plan === newPlan) return; 
    
    setIsLoading(true);
    setUpgradeError("");

    try {
      await api.put('/subscription/upgrade', { newPlan });
      
      // Note: Make sure your Context API exposes an update method 
      // (e.g., updateVendor({ ...vendor, plan: newPlan })) to update global state here.
      // For now, reloading the window ensures the app pulls the freshest state.
  ...
  const limit = getProductLimit(currentPlan);
  const usagePercentage = limit !== "Unlimited" ? Math.min(100, (productCount / limit) * 100) : 0;
  const isLimitReached = limit !== "Unlimited" && productCount >= limit;

  // --- HARDCODED FOR DEMO ---
  const invoices = [
    { id: "INV-11002", date: "Jun 12, 2026", amount: "₦4,499", plan: "Starter Plan", status: "Paid" },
    { id: "INV-10945", date: "May 12, 2026", amount: "₦4,499", plan: "Starter Plan", status: "Paid" },
  ];

  if (isAuthLoading || !vendor) return null; // Prevent UI flicker before redirect

  return (
    <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 pb-24 lg:pb-12 w-full">
      <div className="max-w-7xl mx-auto space-y-6 w-full animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
           ...
           <div className={`lg:col-span-7 space-y-6 ${activeTab === 'overview' ? 'block' : 'hidden lg:block'}`}>
              ...
              <div className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-200 shadow-sm">
                 ...
                 <div className="space-y-4">
                   <div>
                     <div className="flex justify-between items-end mb-2">
                       <div>
                         <p className="text-sm font-bold text-gray-900">Products ({currentPlan} Plan)</p>
                         <p className="text-xs font-medium text-gray-500">You are using {productCount} out of {limit} product slots.</p>
                       </div>
                       <span className={`text-sm font-extrabold ${isLimitReached ? 'text-red-500' : 'text-sabi-primary'}`}>
                         {usagePercentage.toFixed(0)}%
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {/* Starter Plan Card */}
                <div className={`rounded-3xl p-6 border-2 transition-all relative ${currentPlan === "STARTER" ? "border-sabi-primary bg-emerald-50/30" : "border-gray-200 bg-white"}`}>
                  {currentPlan === "STARTER" && (
                    <span className="absolute top-0 right-6 -translate-y-1/2 bg-sabi-primary text-white text-[10px] font-extrabold uppercase px-2 py-1 rounded-md">
                      Current Plan
                    </span>
                  ...
                  <h4 className="text-lg font-extrabold text-gray-900">Starter Plan</h4>
                  <div className="mt-2 mb-4">
                    <span className="text-2xl font-black text-gray-900">₦4,499</span>
                    <span className="text-xs font-bold text-gray-500">/month</span>
                  </div>
                  <ul className="space-y-3 mb-6">
                    <li className="flex items-start gap-2 text-sm font-medium text-gray-700">
                  ...
                  <button 
                    ...
                    className={`w-full py-3 rounded-xl font-bold transition-all flex items-center justify-center gap-2 ${currentPlan === "STARTER" ? "bg-emerald-100 text-emerald-700 cursor-not-allowed" : "bg-sabi-primary hover:bg-sabi-primaryDark text-white shadow-md"}`}
                  >
                    {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : currentPlan === "STARTER" ? "Active" : "Upgrade to Starter"}
                  </button>
                </div>

                {/* Growth Plan Card */}
                <div className={`rounded-3xl p-6 border-2 transition-all relative overflow-hidden ${currentPlan === "GROWTH" ? "border-gray-900 bg-gray-900 text-white" : "border-[#044e3b] bg-[#044e3b] text-white"}`}>
                  {currentPlan === "GROWTH" && (
                    <span className="absolute top-0 right-6 -translate-y-1/2 bg-yellow-400 text-yellow-900 text-[10px] font-extrabold uppercase px-2 py-1 rounded-md">
                      Current Plan
                    </span>
                  ...
                  <h4 className="text-lg font-extrabold relative z-10">Growth Plan</h4>
                  <div className="mt-2 mb-4 relative z-10">
                    <span className="text-2xl font-black">₦9,499</span>
                    <span className="text-xs font-bold text-emerald-100">/month</span>
                  </div>
                  <ul className="space-y-3 mb-6 relative z-10">
                    <li className="flex items-start gap-2 text-sm font-medium text-emerald-50">
                    ...
                    <li className="flex items-start gap-2 text-sm font-medium text-emerald-50">
                      <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" /> Remove Branding
                    </li>
                    <li className="flex items-start gap-2 text-sm font-medium text-emerald-50">
                      <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" /> Advanced Analytics
                    </li>
                  ...
                  <button 
                    ...
                    className={`w-full py-3 rounded-xl font-bold transition-all relative z-10 flex items-center justify-center gap-2 ${currentPlan === "GROWTH" ? "bg-white/10 text-white cursor-not-allowed" : "bg-white text-[#044e3b] hover:bg-emerald-50 shadow-md"}`}
                  >
                    {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : currentPlan === "GROWTH" ? "Active" : "Upgrade to Growth"}
                  </button>
                </div>
              </div>
