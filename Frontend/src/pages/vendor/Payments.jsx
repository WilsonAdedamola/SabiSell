...
const Payments = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const banksRes = await api.get('/vendors/banks');
        const rawBankList = banksRes.data.data || [];
  ...
  const handleSaveBankDetails = async (e) => {
    ...
    try {
      ...
      // Reload to ensure Context API pulls fresh profile data
  ...
  const getFeeDetails = () => {
    switch(vendorPlan) {
      case "GROWTH": return { sabi: "0%", total: "1.5% + ₦100" };
      case "STARTER": return { sabi: "1.5% (Capped at ₦3,000)", total: "3% + ₦100" };
      default: return { sabi: "3.0%", total: "4.5% + ₦100" };
    }
  };
  const fees = getFeeDetails();

  if (isAuthLoading || !vendor) return null; // Prevent flicker
  if (isLoading) return (
    <div className="flex-1 flex items-center justify-center bg-gray-50"><div className="w-8 h-8 border-4 border-sabi-primary border-t-transparent rounded-full animate-spin"></div></div>
  );

  return (
    <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 pb-24 lg:pb-12 w-full bg-[#FAF9F5]">
        {toast && (
                <Toast 
                  message={toast.message} 
                  type={toast.type} 
              ...
              <div className="space-y-3">
                <div className={`p-4 rounded-2xl border ${vendorPlan === 'FREE' ? 'bg-white border-blue-200 shadow-sm ring-1 ring-blue-100' : 'bg-white/60 border-transparent'}`}>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="font-black text-gray-900">3.0% <span className="text-xs text-gray-500 font-medium">Platform Fee</span></span>
                  </div>
                  <p className="text-xs text-gray-500">Total deduction per sale: 4.5% + ₦100</p>
                </div>
                ...
                <div className={`p-4 rounded-2xl border ${vendorPlan === 'STARTER' ? 'bg-white border-blue-200 shadow-sm ring-1 ring-blue-100' : 'bg-white/60 border-transparent'}`}>
                  ...
                  <p className="text-xs text-gray-500">Total deduction per sale: 3% + ₦100 (SabiSell fee capped at ₦3,000)</p>
                </div>
