...
const Settings = () => {
  ...
  // Scroll to top on mount and view changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (!formData.storeLink || activeView !== "store-info" || !vendor) return;
    ...
    return () => clearTimeout(debounceTimer);
  }, [formData.storeLink, vendor?.storeLink, activeView, vendor]);
  ...
  const handleToggleStoreStatus = async () => {
    ...
    try {
      ...
      await api.put('/vendors/settings', data);
      
      // Reload to pull fresh context
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Force a reload to sync updated data with AuthContext seamlessly
  ...
  return (
    <div ref={scrollContainerRef} className="flex-1 overflow-y-auto bg-gray-50/50 w-full h-full relative">
      <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 pb-32 sm:pb-12">
