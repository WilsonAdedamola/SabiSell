...
const StoreLink = () => {
  ...
  useEffect(() => {
    ...
    if (vendor && vendor.storeLink) {
      // Construct the correct URL depending on environment
      const isFreeHost = window.location.hostname.includes('vercel.app') || window.location.hostname.includes('localhost');
      const url = isFreeHost 
        ? `${window.location.origin}/store/${vendor.storeLink}` 
        : `${window.location.protocol}//${vendor.storeLink}.${window.location.host.replace('www.', '')}`;
        
      setStoreUrl(url);
    }
  }, [vendor, isAuthLoading]);
  ...
  const handleNativeShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Shop at ${vendor?.storeName || 'my store'}`,
          text: `Check out my products on SabiSell!`,
          url: storeUrl,
        });
      } catch (err) {
  ...
  const downloadQRCode = () => {
    ...
    let downloadLink = document.createElement("a");
    downloadLink.href = pngUrl;
    downloadLink.download = `${vendor?.storeName?.replace(/\s+/g, '_') || 'Store'}_QRCode.png`;
    document.body.appendChild(downloadLink);
    downloadLink.click();
    document.body.removeChild(downloadLink);
  };
  ...
  return (
    <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 pb-24 lg:pb-12 w-full relative bg-sabi-surface">
      <div className="w-full max-w-7xl mx-auto space-y-6">
